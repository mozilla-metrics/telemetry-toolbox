...
mainthreads = FOREACH filtered_main GENERATE (chararray)json_map#'info'#'appName' AS product:chararray,
...
plus_avg_time_main = FOREACH mainthreads GENERATE product,product_version,product_channel,sql,count,t,((double)t / (double)count) AS avg_time:double;
grouped_main = GROUP plus_avg_time_main BY (product,product_version,product_channel,sql);
...
slowsql_main = FOREACH grouped_main {
    ...
    GENERATE FLATTEN(group) AS (product,product_version,product_channel,sql),
             COUNT(plus_avg_time_main) AS doc_count,
...
otherthreads = FOREACH filtered_other GENERATE (chararray)json_map#'info'#'appName' AS product:chararray,
                                               (chararray)json_map#'info'#'appVersion' AS product_version:chararray,
                                               (chararray)json_map#'info'#'appUpdateChannel' AS product_channel:chararray,
                                               FLATTEN(SlowSqlTuples(json_map#'slowSQL'#'otherThreads')) AS (sql:chararray, count:long, t:long);
plus_avg_time_other = FOREACH otherthreads GENERATE product,product_version,product_channel,sql,count,t,((double)t / (double)count) AS avg_time:double;
grouped_other = GROUP plus_avg_time_other BY (product,product_version,product_channel,sql);
