...
histograms = FOREACH filtered_genmap GENERATE k,SUBSTRING(k,1,9) AS d:chararray,json_map#'info'#'appVersion' AS version:chararray,FLATTEN(com.mozilla.telemetry.pig.eval.HistogramValueTuples(json_map#'histograms')) AS (hist_name:chararray, v:chararray, count:double, sum:long, bucket_count:int, min_range:int, max_range:int, hist_type:int);
filtered_histograms = FILTER histograms BY hist_name == 'CYCLE_COLLECTOR' AND count > 0.0;
relevant_hist_fields = FOREACH filtered_histograms GENERATE k,d,version,hist_name,v,count,sum;
